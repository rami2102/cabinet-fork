...
[2026-04-05] Run heartbeat and job agent sessions in Claude Code headless (-p) mode. The daemon now accepts a `headless` flag on POST /sessions; conversation-runner sets it true for non-manual triggers (heartbeat, job). Manual agent chats stay in interactive PTY mode. This eliminates noise and close the chat once completed.
